...
function dayunEndpointDetail(endpoint) {
...
function dayunMechanicsMarkup(step) {
  if (!step) return "";
  ...
  const endpoints = Array.isArray(step.relations_to_natal_endpoints)
    ? step.relations_to_natal_endpoints.map(dayunEndpointDetail)
    : [];
  ...
  return `<section class="dayun-mechanics" aria-label="${esc(step.ganzhi || "所选")}大运机械事实">
    ...
    <div class="dayun-mechanics-group"><span>与原局关系</span><div class="rel-chips">${relationMarkup}</div></div>
...
function renderDayunTimeline(selectedGz = "", selectedYear = "") {
  ...
  renderAdvancedStage(selectedStep);
...
function renderAdvancedStage(selectedStep = null) {
  const p = lastPayload;
  const wrap = $("#advanced-stage");
  if (!p.da_yun) { wrap.innerHTML = ""; return; }
  const step = selectedStep
    || dayunStep(p.da_yun.current || p.da_yun.next || p.da_yun.list?.[0]?.ganzhi);
  const cly = p.current_liu_yue;
  const rels = cly
